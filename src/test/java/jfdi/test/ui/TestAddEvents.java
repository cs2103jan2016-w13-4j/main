...
public class TestAddEvents extends TestMain {
    ...
    public void testAddTaskEmpty() {
        ...
        assertEquals("The feedback message does not match the intended result.", Constants.CMD_ERROR_CANT_ADD_EMPTY,
                fbArea.getText());
        assertEquals("The task is accidentally added to the display list as an item.", listSize,
                controller.importantList.size());
        assertEquals("Notification is accidentally updated", notiSize, controller.incompletePlaceHdr);
    }
     ...
     */
    @Test
    public void testAddTaskDuplicated() {
        ...
        assertEquals("The feedback message does not match the intended result.", Constants.CMD_ERROR_CANT_ADD_DUPLICATE,
                fbArea.getText());
        assertEquals("The task is accidentally added to the display list as an item.", listSize,
                controller.importantList.size());
        assertEquals("Notification is accidentally updated", notiSize, controller.incompletePlaceHdr);
